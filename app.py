...
def login_process(uid, password, region="BD"):
    try:
        ...
        # --- ডাইনামিক বাইনারি পেলোড তৈরি (Protobuf Format) ---
        part1 = b'\x1a\x132025-08-30 05:19:21"\tfree fire(\x01'
        
        client_version_bytes = CLIENT_VERSION.encode("ascii")
        version_payload = b':' + bytes([len(client_version_bytes)]) + client_version_bytes
        
        os_info = f"{ANDROID_OS_VERSION} / API-28 (PI/rel.cjw.20220518.114133)"
        os_info_bytes = os_info.encode("ascii")
        os_payload = b'B' + bytes([len(os_info_bytes)]) + os_info_bytes
        
        part2 = b'J\x08HandheldR\nATM MobilsZ\x04WIFI`\xb6\nh\xee\x05r\x03300z\x1fARMv7 VFPv3 NEON VMH | 2400 | 2\x80\x01\xc9\x0f\x8a\x01\x0fAdreno (TM) 640\x92\x01\rOpenGL ES 3.2\x9a\x01+Google|dfa4ab4b-9dc4-454e-8065-e70c733fa53f\xa2\x01\x0e105.235.139.91\xaa\x01\x02'
        
        binary_head = part1 + version_payload + os_payload + part2
...
def get_jwt_token():
    ...
    # যদি টোকেন আগে থেকেই থাকে এবং মেয়াদ (৫ ঘণ্টা) শেষ না হয়ে থাকে, তাহলে সেটাই ব্যবহার করবে
    ...
    for attempt in range(1, max_retries + 1):
        print(f"Generating JWT token... (Attempt {attempt}/{max_retries})")
        token = login_process(target_uid, target_pass, "BD")
        
        if token:
            jwt_token = token
            # ৫ ঘণ্টার জন্য টোকেনের মেয়াদ সেট করা হলো (৫ * ৩৬০০ সেকেন্ড)
            token_expiry = time.time() + (5 * 3600) 
            return jwt_token
        else:
